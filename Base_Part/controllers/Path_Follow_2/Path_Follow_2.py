# ---------------- YouBot Alignment + A* Path Planning + Final Orientation (Scene-tree) ----------------
...
if node is None:
    raise SystemExit("ERROR: DEF YOUBOT not found in scene tree.")
...
# ---------------- Fetch target and obstacles ----------------
target_prefixes = ["BALL1"]
TARGET_POINT = find_first_node_translation_by_prefix(target_prefixes)
if not TARGET_POINT:
    raise SystemExit(f"ERROR: No TARGET node found (expected DEF starting with {target_prefixes})")

obstacle_prefixes = ["BALL2"]
obstacles_found = find_all_node_translations_by_prefix(obstacle_prefixes)
if not obstacles_found:
    raise SystemExit(f"ERROR: No obstacle nodes found (expected DEF starting with {obstacle_prefixes})")

# Keep only x,y for grid
obstacles = [[t[0], t[1]] for t in obstacles_found]

print(f"Target from scene tree: {TARGET_POINT}")
print(f"Obstacles from scene tree: {obstacles}")
...
class Cell:
    def __init__(self):
        self.parent_i=0; self.parent_j=0; self.f=float('inf'); self.g=float('inf'); self.h=0
