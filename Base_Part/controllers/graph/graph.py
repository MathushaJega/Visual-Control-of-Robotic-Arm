...
# -------------------------------------------------
#   Graph Setup (Matplotlib Interactive Mode)
# -------------------------------------------------
plt.ion() # Turn on interactive mode
fig, ax = plt.subplots(figsize=(8, 5))
fig.canvas.manager.set_window_title('Robot Accuracy')
...
while robot.step(timestep) != -1:
    ...
    cur_yaw, tgt_yaw = current_yaw_and_target_angle()
    
    # Calculate current distance to the absolute target for the graph
    dx_target = TARGET_POINT[0] - robot_pos[0]
    dy_target = TARGET_POINT[1] - robot_pos[1]
    dist_to_target = math.hypot(dx_target, dy_target)
    err_yaw = wrap_pi(tgt_yaw - cur_yaw)

    # Record data for graphing
    time_data.append(current_time)
    dist_error_data.append(dist_to_target)
    yaw_error_data.append(abs(err_yaw))

    # Update the plot every 10 steps to prevent simulation lag
    if step_counter % 10 == 0:
        ax.clear()
        ax.set_title("Robot Tracking Accuracy Over Time")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Error (m or rad)")
        ax.plot(time_data, dist_error_data, label="Distance to Target (m)", color='blue')
        ax.plot(time_data, yaw_error_data, label="Absolute Yaw Error (rad)", color='red')
        ax.legend(loc="upper right")
        ax.grid(True)
        plt.pause(0.001)

    # Always assume path exists
    if path_cells and len(path_cells) > 2:  # stop one cell before last
...
while robot.step(timestep) != -1:
    ...
    # Continue recording and plotting data during final alignment
    time_data.append(current_time)
    dist_error_data.append(dist_to_target) 
    yaw_error_data.append(abs(final_err))

    if step_counter % 10 == 0:
        ax.clear()
        ax.set_title("Robot Tracking Accuracy (Final Alignment)")
        ax.set_xlabel("Time (s)")
        ax.set_ylabel("Error (m or rad)")
        ax.plot(time_data, dist_error_data, label="Distance to Target (m)", color='blue')
        ax.plot(time_data, yaw_error_data, label="Absolute Yaw Error (rad)", color='red')
...
# Keep the plot open at the end of the simulation
print("Simulation complete. Close the graph window to exit.")
plt.ioff()
plt.show()