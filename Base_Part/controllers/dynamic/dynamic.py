...
def get_dynamic_obstacles():
    obs_nodes = find_all_node_translations_by_prefix(["BALL2"])
...
class DStarLite:
    ...
    def update_grid(self,new_grid):
        # Detect cells whose occupancy changed
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
...
while robot.step(timestep) != -1:
    ...
    if not path_cells:
        stop_all()
        print("No path available. Waiting for obstacles to move...")
        continue

    # Follow only next waypoint of the path
    if len(path_cells) > 1:
